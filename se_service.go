...
// SEHeartbeatData is used internally to pass data from the service to the API handler
type SEHeartbeatData struct {
	PinTries     int
	SeVersion    string
	IsVerified   bool
...
func (s *SecureElementService) connectAndSelect(readerName string) (*scard.Context, *scard.Card, error) {
	...
	if verified && pin != "" {
		...
		if err != nil {
			s.ClearSession()
			s.cleanup(ctx, card)
			return nil, nil, fmt.Errorf("auto-verification transmit failed")
		}
		if err := CheckAPDUError(resp); err != nil {
			s.ClearSession()
			s.cleanup(ctx, card)
			return nil, nil, fmt.Errorf("auto-verification failed: %w", err)
		}
	}
...
func (s *SecureElementService) GetHeartbeatData(readerName string) (*SEHeartbeatData, error) {
	// 1. Connect Raw (No Auto-Verify yet)
	ctx, card, version, err := s.connectRaw(readerName)
	if err != nil {
		return nil, err
	}
	...
	data := &SEHeartbeatData{}
	data.SeVersion = fmt.Sprintf("%d.%d.%d", version.Major, version.Minor, version.Patch)

	// 2. Get PIN Tries
	if tries, err := s.getPinTries(card); err == nil {
		data.PinTries = tries
	}
	...
	// 4. If verified locally, verify on card to be sure
	if data.IsVerified && pin != "" {
		pinBytes := s.encodePin(pin, version)
		apduVerify := make([]byte, 0)
		apduVerify = append(apduVerify, CmdVerifyPinHeader...)
		apduVerify = append(apduVerify, byte(len(pinBytes)))
		...
		if resp, err := card.Transmit(apduVerify); err != nil || CheckAPDUError(resp) != nil {
			data.IsVerified = false
			s.ClearSession() // Reset if card rejected it
		}
	}

	// 5. Fetch Protected Data if Verified
	if data.IsVerified {
		// Amount Status
		if status, err := s.getAmountStatus(card); err == nil {
			data.AmountStatus = status
		}

		// Cert Params (UID)
		if params, err := s.getCertParams(card); err == nil {
			data.CertParams = params
		}

		// Taxpayer Info (Use Cache if available, else fetch)
		if cachedInfo != nil {
			data.TaxpayerInfo = cachedInfo
		} else {
			if info, err := s.getTaxpayerInfo(card); err == nil {
				data.TaxpayerInfo = info
				// Update Cache
				s.mu.Lock()
				s.cachedInfo = info
				s.mu.Unlock()
...
// --- Internal Logic (Decoupled from Connection) ---

func (s *SecureElementService) getPinTries(card *scard.Card) (int, error) {
...
func (s *SecureElementService) getAmountStatus(card *scard.Card) (*AmountStatus, error) {
	...
	return &AmountStatus{
		SumAmount:   fmt.Sprintf("%.4f", float64(binary.BigEndian.Uint64(sumBytes))/10000.0),
		LimitAmount: fmt.Sprintf("%.4f", float64(binary.BigEndian.Uint64(limitBytes))/10000.0),
	}, nil
}

func (s *SecureElementService) getCertParams(card *scard.Card) (*CertParams, error) {
	...
	return &CertParams{
		UID:       string(uidBytes),
		NotBefore: time.UnixMilli(int64(notBeforeMs)).Format(time.RFC3339),
		NotAfter:  time.UnixMilli(int64(notAfterMs)).Format(time.RFC3339),
	}, nil
}

func (s *SecureElementService) getTaxpayerInfo(card *scard.Card) (*TaxpayerInfo, error) {
	...
	// 2. Parse Extensions (TIN/URL)
	ekuOID := asn1.ObjectIdentifier{2, 5, 29, 37}
	var tinOID, urlOID []int
...
func (s *SecureElementService) VerifyPin(readerName string, pin string) (int, error) {
	...
	// We check SW manually here to return it to the API
	sw := 0
	if len(resp) >= 2 {
...
func (s *SecureElementService) GetTaxpayerInfo(readerName string) (*TaxpayerInfo, error) {
	...
	info, err := s.getTaxpayerInfo(card)
	if err == nil {
		s.mu.Lock()
		s.cachedInfo = info
		s.mu.Unlock()
...
func (s *SecureElementService) parseInvoiceResponse(resp []byte) (*LastSignedInvoice, error) {
	...
	readableAmt := strconv.FormatFloat(float64(rawAmt)/10000.0, 'f', 4, 64)
